...
#openai.api_key = os.getenv("OPENAI_API_KEY")w
IMPORT_HEADER = "from typing import *\nimport math\nfrom heapq import *\nimport itertools\nimport re\nimport typing\nimport heapq\n_str=str\nimport re\nimport hashlib\nimport heapq\nimport collections\nfrom collections import *\nfrom itertools import combinations\nfrom math import prod\nfrom itertools import combinations_with_replacement\nfrom  decimal import Decimal, getcontext\nimport numpy as np\n"

def regex_escape_special(text: str) -> str:
    regex= ""
    for c in text:
        regex+= re.escape(c)
...
def read_jsonl(path: str, lineLimit:int | None = None) -> List[Dict[str, Any]]:
    ...
    with jsonlines.open(path) as reader:
        for item in reader:
            items.append(item)
            linesRead+=1
            if lineLimit is not None and linesRead >= lineLimit:
                break
    return items

def write_jsonl(path: str, data: List[Dict[str, str]], append: bool = False):
    with jsonlines.open(path, mode='a' if append else 'w') as writer:
        for item in data:
            writer.write(item)

def write_line(path: str, data: str):
    with jsonlines.open(path, mode='a') as writer:
        writer.write(data)

def read_jsonl_gz(path: str) -> list[dict[str, str]]:
